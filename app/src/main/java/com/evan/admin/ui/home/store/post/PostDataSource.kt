...
class PostDataSource (val context: Context, val alertRepository: HomeRepository) :
    ...
    override fun loadInitial(
        ...
        Coroutines.main {
            ...
            try {
                ...
                post = LimitPost(10, 1)
                val response = alertRepository.getInActivePost(SharedPreferenceUtil.getShared(context, SharedPreferenceUtil.TYPE_AUTH_TOKEN)!!,post!!)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Post>) {
        Coroutines.main {
            try {
                ...
                val response =
                    alertRepository.getInActivePost(SharedPreferenceUtil.getShared(context, SharedPreferenceUtil.TYPE_AUTH_TOKEN)!!,post!!)
                response.success.let { isSuccessful ->
                    if (isSuccessful!!) {
                        val nextPageKey =
                            params.key + 1
