...
class CustomerDataSource (val context: Context, val alertRepository: HomeRepository) :
    ...
    override fun loadInitial(
        ...
        Coroutines.main {
            ...
            try {
                ...
                post = LimitPost(10, 1)
                val response = alertRepository.getCustomerList(SharedPreferenceUtil.getShared(context, SharedPreferenceUtil.TYPE_AUTH_TOKEN)!!,post!!)
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, Customer>) {
        Coroutines.main {
            try {
                ...
                val response =
                    alertRepository.getCustomerList(SharedPreferenceUtil.getShared(context, SharedPreferenceUtil.TYPE_AUTH_TOKEN)!!,post!!)
                response.success.let { isSuccessful ->
                    if (isSuccessful!!) {
                        val nextPageKey =
                            params.key + 1
